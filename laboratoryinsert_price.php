...
if (isset($_SESSION['id']) && isset($_SESSION['uname'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        header("Location: laboratorytest_form.php?success=$testName updated successfully.");
        exit();
            
            } else {
                        // Redirect to the form page with appropriate message
                        header("Location: laboratorytest_form.php?error=Error updating $testName.");
                        exit();
            }
        } else {
            ...
            if ($stmt->affected_rows > 0) {
                        // Redirect to the form page with appropriate message
        header("Location: laboratorytest_form.php?success=$testName added successfully.");
        exit();
            
            } else {
                header("Location: laboratorytest_form.php?error=Error adding $testName.");
        exit();
